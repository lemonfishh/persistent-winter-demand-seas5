...
#
# Research question:
#   How often do uninterrupted cold spells persist to durations
#   comparable with the 21--35-day demand averaging windows?
#
...
# Exact values reported in the compact Results table.
KEY_SUMMARY_DURATIONS = [21, 28]
...
OUT_KEY_SUMMARY = (
    OUT_DIR
    / "raw_temperature_cold_spell_key_summary.csv"
)

OUT_ESTIMATES = (
    OUT_DIR
    / "raw_temperature_duration_reach_estimates.csv"
)
...
OUT_FIGURE_PNG = (
    OUT_DIR
    / "raw_temperature_cold_spell_duration.png"
)

OUT_FIGURE_PDF = (
    OUT_DIR
    / "raw_temperature_cold_spell_duration.pdf"
)
...
def calculate_duration_metrics(
    ...
    """
    Calculate the proportion of retained spells reaching each duration.
    """
    ...
    for duration in DURATION_REACH_DAYS:
        numerator = int(
            np.sum(
                durations >= duration
            )
...
# ============================================================
# Original duration-reach estimates
# ============================================================
...
# ============================================================
# Compact summary for the Results table
# ============================================================

key_summary = threshold_summary[
    [
        "threshold_percentile",
        "threshold_label",
        "threshold_temperature_C",
        "n_events_at_least_3_days",
        "maximum_event_duration_days",
    ]
].copy()

for duration in KEY_SUMMARY_DURATIONS:
    duration_summary = duration_counts[
        duration_counts["duration_days"] == duration
    ][
        [
            "threshold_percentile",
            "n_events_at_least_duration",
            "n_events_total",
        ]
    ].copy()

    duration_summary[
        f"proportion_reaching_{duration}_days_percent"
    ] = (
        100.0
        * duration_summary["n_events_at_least_duration"]
        / duration_summary["n_events_total"]
    )

    duration_summary = duration_summary.rename(
        columns={
            "n_events_at_least_duration": (
                f"n_events_reaching_{duration}_days"
            ),
        }
    ).drop(columns="n_events_total")

    key_summary = key_summary.merge(
        duration_summary,
        on="threshold_percentile",
        how="left",
        validate="one_to_one",
    )
...
# ============================================================
# Main figure: duration-reach proportions only
# ============================================================
...
duration_reach_data = bootstrap_ci[
    bootstrap_ci["metric"] == "duration_reach"
].copy()

for percentile in TEMPERATURE_PERCENTILES:
    threshold_data = duration_reach_data[
        duration_reach_data["threshold_percentile"] == percentile
    ].sort_values("duration_days")

    line = ax.plot(
        threshold_data["duration_days"],
        threshold_data["original_estimate"],
        marker="o",
        linewidth=1.6,
        label=(
            f"Temperature <= "
    ...
    ax.fill_between(
        threshold_data["duration_days"].to_numpy(dtype=float),
        threshold_data["ci_lower"].to_numpy(dtype=float),
        threshold_data["ci_upper"].to_numpy(dtype=float),
        alpha=0.16,
        color=line.get_color(),
    )

ax.set_xlabel("Duration reached (days)")
ax.set_ylabel("Proportion of retained cold spells")
ax.set_title("Proportion of cold spells reaching each duration")
ax.set_ylim(-0.03, 1.03)
ax.set_xticks(DURATION_REACH_DAYS)
ax.grid(True, alpha=0.3)
ax.legend(fontsize=8.5)

fig.suptitle(
    "SEAS5 raw-temperature cold-spell duration\n"
    "Unsmoothed daily temperature; shaded areas show 95% "
    "winter-year bootstrap intervals",
    fontsize=12.5,
    y=1.02,
)
